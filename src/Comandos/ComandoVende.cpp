...
ComandoVende::ComandoVende() : Comando("vende", "Vende toda a mercadoria da caravana N, o qual devera estar numa cidade.","<N>") { }
    ...
    if (aux) {
        if(aux->getCidadeName() == ' ') {
            ...
            if(aux->getMercadoria() != 0) {
                ...
                cout << "Utilizador vendeu " << aux->getMercadoria() << " toneladas de mercadoria da caravana " << aux->getID() << endl;
                aux->setMercadoria(0);
            } else {

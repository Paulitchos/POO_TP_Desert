...
ComandoCompra::ComandoCompra() : Comando("compra","Compra M toneladas de mercadorias para a caravana N, o qual deverá estar numa cidade nessa altura", "<N> <M>"){ }
    ...
    if (aux) {
        if(!aux->getInCity()) {
            ...
            if(aux->adicionaMercadoria(aux->getMercadoria() + mercadoria) && sim.getMapBuyMerch() * mercadoria <= sim.getMapCoins()) {
                ...
                cout << "A carava " << aux->getID() << " ficou com " << aux->getMercadoria() << " toneladas de mercadoria" << endl;

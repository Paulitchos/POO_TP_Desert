...
ComandoAreia::ComandoAreia() : Comando("areia", "Cria uma tempestade de areia na posicao l, c com raio r posicoes",
                                       "<l> <c> <r>") {
}
...
void ComandoAreia::execute(const string &args, Interface *interface) {
    ...
    if (inputs.size() != 4) {
        cout << "O comando " << getNome() << " apenas aceita tres argumentos <l> que e um numero e corresponde "
                ...
                " <r> e o raio da tempestade!!" << endl << endl;
        return;
    }
    ...
    if (!isNumeric(inputs[3]) || stoi(inputs[3]) < 0 || stoi(inputs[3]) > maximoRaio) {
        cout << "argumento <r> precisa de ser um numero e pode ultrapassar o tamanho do mapa!!" << endl << endl;
        return;
    }
    ...
    if (raio == maximoRaio) {
        raio = (raio / 2);
        cout << "Devido a tempestade ter o numero igual a um dos limites do mapa, foi diminuada a 1/2 do seu valor" <<
                endl << endl;
    }

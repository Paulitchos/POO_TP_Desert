...
void Pandora::execute(shared_ptr<Caravana> car, Barbaro *bar) {
    if (car != nullptr) {
        if (car->getNPessoas() != 0) {
            car->removePessoas(static_cast<int>(car->getNPessoas() * 0.2));
            cout << "Encontrou uma " << getNome() <<
                    ", 20% das pessoas da caravana foram eliminadas. Pessoas Restantes: " <<
                    car->getNPessoas() <<
                    endl;
        } else {
    ...
    if (bar != nullptr) {
        if (bar->getNPessoas() != 0) {
            bar->removePessoas(static_cast<int>(bar->getNPessoas() * 0.2));
            cout << "Encontrou uma " << getNome() <<
                    ", 20% das pessoas da caravana barbara foram eliminadas. Pessoas Restantes: " <<
